...
declare interface mxUtils {
   ...
   */
  errorResource: string; //(mxClient.language != 'none') ? 'error' : '',
   ...
   */
  closeResource: string; //(mxClient.language != 'none') ? 'close' : '',
   ...
   */
  errorImage: string; //mxClient.imageBasePath + '/error.gif',
   ...
   */
  findNode(node: Node, attr: string, value: any);
   ...
   */
  // TODO
  // check if type object is OK, how it EXACTELY compares to "any" type
  remove(obj: object, array: object[]): object[];
   ...
   */
  isAncestorNode(ancestor: Node, child): boolean;
   ...
   */
  getChildNodes(node: Node, nodeType?);
   ...
   */
  importNode(doc: Document, node: Node, allChildren: boolean);
   ...
   */
  getPrettyXml(node: Node, tab?: string, indent?: string);
   ...
   */
  extractTextWithWhitespace(elems: Node[]): string;
   ...
   */
  getTextContent(node: Node): boolean;
   ...
   */
  br(parent: Node, count?: number): HTMLBRElement;
   ...
   * current document.
   */
  button(label: string, funct, doc?: Document): HTMLButtonElement;
   ...
   */
  // TODO
  // is 'p' element a HTMLParagraphElement???
  para(parent: Node, text): HTMLParagraphElement;
   ...
   */
  addTransparentBackgroundFilter(node: Node): void;
   ...
   */
  // linkAction(parent: Node, text: string, editor: mxEditor, action: string, pad?: number);
   ...
   * pad - Optional left-padding for the link. Default is 0.
   */
  // linkInvoke(parent: Node, text: string, editor: mxEditor, functName: string, arg: object, pad?: number): HTMLSpanElement;
   ...
   * pad - Optional left-padding for the link. Default is 0.
   */
  link(parent: Node, text: string, funct, pad?: number): HTMLSpanElement;
   ...
   */
  // get(url: string, onload?, onerror?, binary?: boolean, timeout?: number, ontimeout?): mxXmlRequest;
   ...
   * onerror - Optional function to execute on error.
   */
  getAll(urls: string[], onload, onerror?): void;
   ...
   * onerror - Optional function to execute on error.
   */
  // post(url: string, params: any[], onload?, onerror?): mxXmlRequest;
   ...
   */
  // submit(url: string, params: any[], doc: Document, target): mxXmlRequest;
   ...
   */
  loadInto(url: string, doc: Document, onload): void;
   ...
   * key is null.
   */
  getValue(array: any[], key: string, defaultValue: any): any | null;
   ...
   */
  getNumber(array: any[], key: string, defaultValue?: any): any;
   ...
   */
  getColor(array: any[], key: string, defaultValue: any): any | null;
   ...
   */
  equalEntries(a: object, b: object): boolean;
   ...
   */
  arcToCurves(x0: number, y0: number, r1: number, r2: number, angle: number, largeArcFlag, sweepFlag, x: number, y: number): number[];
   ...
   */
  getPortConstraints(terminal: mxCellState, edge: mxCellState, source: boolean, defaultValue: any | null): any | null;
   ...
   */
  findNearestSegment(state, x: number, y: number): number;
   ...
   * b - <mxRectangle> to be checked for intersection.
   */
  intersectsHotspot(state, x: mxRectangle, y: mxRectangle, hotspot: number, min: number, max: number): boolean;
   ...
   */
  intersection(x0: number, y0: number, x1: number, y1: number, x2: number, y2: number, x3: number, y3: number): mxPoint | null;
   ...
   */
  animateChanges(graph: mxGraph, changes): void;
   ...
   * will be removed later.
   */
  fadeOut(node: Node, from, remove, step, delay, isEnabled: boolean): void;
   ...
   */
  indexOfStylename(style: string | null, stylename: string | null): number;
   ...
   */
  addStylename(style: string | null, stylename: string | null): string;
   ...
   */
  removeStylename(style: string | null, stylename: string | null): string;
   ...
   */
  setCellStyles(model: mxGraphModel, cells: mxCell[], key: string, value: any): void;
   ...
   * value - New value for the given key.
   */
  setStyle(style: string, key: string, value: any): string;
   ...
   */
  getAlignmentAsPoint(align, valign): mxPoint;
   ...
   * Function: getViewXml
   */
  getViewXml(graph: mxGraph, scale?: number, cells?: mxCell[], x0?: number, y0?: number);
   ...
   */
  error(message: string, width: number, close?: boolean, icon?): mxWindow;
